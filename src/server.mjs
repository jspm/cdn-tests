...
export class Server extends EventEmitter {
  ...
  constructor(port = 8080, listeningCb = null) {
    super();
    this.port = port;
    this.server = http
      .createServer(async (req, res) => {
        ...
        // For debugging CI:
        if (process.env.CI_BROWSER) console.log("Received request: " + url);
  ...
  async runTests(testData) {
    ...
    try {
      ...
      const testCompletion = new Promise((resolve, reject) => {
        ...
        timeout = setTimeout(
          () => reject(new Error(`test#${id} timed out`)),
          60000
        );
      });
  ...
  async handleFile(req, res) {
    ...
    try {
      ...
      else mime = this.mimes[path.extname(filePath)] || "text/plain";
